class Battle::Field_corruptedcave < Battle::Field
  def initialize(battle, duration = Battle::Field::DEFAULT_FIELD_DURATION)
    super
    @id                  = :corruptedcave
    @name                = _INTL("Corrupted Cave")
    @nature_power_change = :GUNKSHOT
    @mimicry_type        = :POISON
    @camouflage_type     = :POISON
    @terrain_pulse_type  = :POISON
    @secret_power_effect = 2 # need to change to poison
    @shelter_type        = :POISON
    @field_announcement  = { :start => _INTL("Corruption seeps from every crevice!"),
                             :end   => _INTL("The cave is no longer corrupted") }

  end
end

Battle::Field.register(:corruptedcave, {
  :trainer_name => [],
  :environment  => [],
  :map_id       => [],
  :edge_type    => [],
