...
=begin #this WAS such a bad ability omg glad we made it #based as fuck
...
Battle::AbilityEffects::MoveImmunity.add(:DETOXIFICATION,
  proc { |ability, user, target, move, type, battle, show_message|
    next target.pbMoveImmunityStatRaisingAbility(user, move, type,
...
Battle::AbilityEffects::AccuracyCalcFromUser.add(:ILLUMINATE,
  proc { |ability, mods, user, target, move, type|
    mods[:accuracy_multiplier] *= 1.3 if user.speed < target.speed
  }
)
...
Battle::AbilityEffects::AccuracyCalcFromUser.add(:FROZENAURA,
  proc { |ability, mods, user, target, move, type|
...
Battle::AbilityEffects::AccuracyCalcFromTarget.add(:FROZENAURA,
  proc { |ability, mods, user, target, move, type|
    mods[:base_accuracy] = 0 if type == :ICE
  }
)
...
Battle::AbilityEffects::DamageCalcFromUser.add(:DEFEATIST,
  proc { |ability, user, target, move, mults, power, type|
    mults[:attack_multiplier] /= 1.5 if target.hp <= target.totalhp / 3
  }
)
...
Battle::AbilityEffects::DamageCalcFromUser.add(:MEGALAUNCHER,
  proc { |ability, user, target, move, mults, power, type|
    mults[:power_multiplier] *= 1.5 if move.pulseMove? || move.bombMove?
  }
)
...
Battle::AbilityEffects::DamageCalcFromUser.add(:RIVALRY,
  proc { |ability, user, target, move, mults, power, type|
    mults[:power_multiplier] *= 1.25 if user.speed >= target.speed
  }
)
...
Battle::AbilityEffects::DamageCalcFromTarget.add(:RUBBERBODY,
  proc { |ability, user, target, move, mults, power, type|
    mults[:power_multiplier] /= 2 if move.physicalMove?
  }
)
...
Battle::AbilityEffects::OnBeingHit.add(:FROSTTOUCH,
  proc { |ability, user, target, move, battle|
    next if !move.pbContactMove?(user)
    ...
    if user.pbCanFreeze?(target, Battle::Scene::USE_ABILITY_SPLASH) &&
      ...
      msg = nil
      if !Battle::Scene::USE_ABILITY_SPLASH
        msg = _INTL("{1}'s {2} Frozen {3}!", target.pbThis, target.abilityName, user.pbThis(true))
      end
      user.pbFreeze(msg)
    end
    battle.pbHideAbilitySplash(target)
...
Battle::AbilityEffects::OnBeingHit.copy(:IRONBARBS, :ROUGHSKIN, :ENSNARINGAURA)
...
Battle::AbilityEffects::OnDealingHit.add(:VAMPYRISM,
  proc { |ability, user, target, move, battle|
  next if !move.contactMove?
  ...
  if target.hasActiveAbility?(:SHIELDDUST) && !battle.moldBreaker
    ...
    battle.pbHideAbilitySplash(target)
  elsif user.pbRecoverHP(user.totalhp / 16) && target.pbCanPoison?(user, Battle::Scene::USE_ABILITY_SPLASH)
  if !Battle::Scene::USE_ABILITY_SPLASH
      battle.pbDisplay(_INTL("{1}'s Vampyric ability drained {2}!", user.pbThis, target.pbThis(true)))
  end
  target.pbPoison(user, _INTL("{1} was poisoned by {2}'s {3}!", target.pbThis, user.pbThis(true), user.abilityName))
end
battle.pbHideAbilitySplash(user)
  }
)

# guarenteed paralysis upon critcal hit
Battle::AbilityEffects::OnDealingHit.add(:TESSENJUTSU,
  proc { |ability, user, target, move, battle|
    next if !target.damageState.critical
    next if !target.pbCanParalyze?(user, Battle::Scene::USE_ABILITY_SPLASH)
    battle.pbShowAbilitySplash(user)
    if !Battle::Scene::USE_ABILITY_SPLASH
      battle.pbDisplay(_INTL("{1}'s fatal points were hit!", target.pbThis))
    end
    battle.pbHideAbilitySplash(user)
    target.pbParalyze(user, _INTL("{1} was paralyzed by {2}'s {3}!", target.pbThis, user.pbThis(true), user.abilityName))
    battle.pbHideAbilitySplash(user)
  }
)

Battle::AbilityEffects::OnDealingHit.add(:STEAMINGCORE,
  proc { |ability, user, target, move, battle|
...
Battle::AbilityEffects::EndOfRoundEffect.add(:DREAMCATCHER,
  proc { |ability, battler, battle|
    battle.allOtherSideBattlers(battler.index).each do |b|
      next if !b.near?(battler) || !b.asleep?
      battle.pbShowAbilitySplash(battler)
    ...
    # use battler.allAllies.each if you want only allies to be healed Feli
    battle.allSameSideBattlers.each do |b|
      #next if battler.allAllies.none? { |b| b.status == :SLEEP } Use this line if you want only allies to be healed
      next if !b.asleep?
      battle.pbShowAbilitySplash(battler)
      b.pbRecoverHP(b.totalhp / 8)
      if Battle::Scene::USE_ABILITY_SPLASH
        battle.pbDisplay(_INTL("{1} is enjoying it's pleasant dreams!", b.pbThis))
      else
        battle.pbDisplay(_INTL("{1} is enojoying the pleasant dreams from {2}'s {3}!",
            b.pbThis, battler.pbThis(true), battler.abilityName))
      end
      battle.pbHideAbilitySplash(battler)
    end
...
Battle::AbilityEffects::TrappingByTarget.add(:ENSNARINGAURA,
  proc { |ability, switcher, bearer, battle|
    next true if !switcher.hasActiveAbility?(:ENSNARINGAURA)
  }
)
