...
Battle::ItemEffects::DamageCalcFromUser.add(:LIFELESSORB,
  proc { |item, user, target, move, mults, power, type|
    if !move.is_a?(Battle::Move::Confusion)
...
Battle::ItemEffects::AfterMoveUseFromUser.add(:LIFELESSORB,
  proc { |item, user, targets, move, numHits, battle|
    next if !user.takesIndirectDamage?
    next if !move.pbDamagingMove? || numHits == 0
    hitBattler = false
    ...
    next if !hitBattler
    PBDebug.log("[Item triggered] #{user.pbThis}'s #{user.itemName} (recoil)")
    user.pbReduceHP(user.totalhp / 3)
    battle.pbDisplay(_INTL("{1} lost lots of its HP from the Lifeless Orb!", user.pbThis))
    user.pbItemHPHealCheck
    user.pbFaint if user.fainted?
  }
)
