class Battle::Battler
  ...
  def pbUseMove(choice, specialUsage = false)
    ...
    if targets.length == 0 && move.pbTarget(user).num_targets > 0 && !move.worksWithNoTargets?
      ...
      # Move-specific effects after all hits on Cave Field (Cave Collapse)
      targets.each { |b| move.pbEffectAfterAllHitsCave(user, b) }
      # Faint if 0 HP
      targets.each { |b| b.pbFaint if b&.fainted? }
      user.pbFaint if user.fainted?
  ...
  def pbProcessMoveHit(move, user, targets, hitNum, skipAccuracyCheck)
    ...
    targets.each do |b|
      ...
      if @battle.pbRandom(100) < chance
        PBDebug.log("[Item/ability triggered] #{user.pbThis}'s King's Rock/Razor Fang or Stench/Specter")
        b.pbFlinch(user)
      end
    end
