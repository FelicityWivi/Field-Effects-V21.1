...
class Battle::Move::OHKO < Battle::Move::FixedDamageMove
  def pbFailsAgainstTarget?(user, target, show_message)
    if !target.hp == (target.hp/10) || target.level > user.level
  ...
  def pbAccuracyCheck(user, target)
    acc = @battle.pbRandom(100) < (@accuracy + user.level - target.level)
    acc = 0 if target.hp == (target.hp/10)
    return acc
  end
...
class Battle::Move::OHKOIce < Battle::Move::OHKO
  def pbFailsAgainstTarget?(user, target, show_message)
    if target.pbHasType?(:ICE) || !target.hp == (target.hp/10)
      @battle.pbDisplay(_INTL("But it failed!")) if show_message
      return true
    end
    return super
...
class Battle::Move::TypeDependsOnUserPlate < Battle::Move
  ...
  def pbBaseType(user)
    ...
    if user.item_id && user.itemActive?
      typ = SuperHardModeEncountersPlatesItemsType[user.item_id]
...
class Battle::Move::TypeDependsOnUserMemory < Battle::Move
  ...
  def pbBaseType(user)
    ret = :NORMAL
    if user.item_id && user.itemActive?
      typ = SuperHardModeEncountersMemoriesItemsType[user.item_id]
      ret = typ if typ && GameData::Type.exists?(typ)
    end
    return ret
  end
...
class Battle::Move::TypeDependsOnUserDrive < Battle::Move
  ...
  def pbBaseType(user)
    ret = :NORMAL
    if user.item_id && user.itemActive?
      typ = SuperHardModeEncountersDrivesItemsType[user.item_id]
      ret = typ if typ && GameData::Type.exists?(typ)
    end
    return ret
  end
...
class Battle::Move::TypeAndPowerDependOnTerrain < Battle::Move
  ...
  def pbBaseType(user)
    ...
    ret = @battle.apply_field_effect(:terrain_pulse_type, user, self)
    return ret if ret
...
#===============================================================================
# Tera Blast and Storm - Repud Version
#===============================================================================

class Battle::Move::RepudTeraBlast < Battle::Move
  def initialize(battle, move)
    super
    @calcCategory = 1
  end
  ...
  def pbBaseType(user)
    ret = :NORMAL
    if user.item_id && user.itemActive?
      typ = SuperHardModeEncountersAllItemsType[user.item_id]
      ret = typ if typ && GameData::Type.exists?(typ)
    end
    return ret
  end
...
class Battle::Move::RepudTeraStarstorm < Battle::Move
  def initialize(battle, move)
    super
    @calcCategory = 1
  end
  ...
  def isStellarTerapagos?(user)
    return user.isSpecies?(:TERAPAGOS) && user.form == 2
  end
  ...
  def pbOnStartUse(user, targets)
    ...
    realAtk = user.attack
    realSpAtk = user.spatk
    @calcCategory = (realAtk > realSpAtk) ? 0 : 1
  end
    
  def pbBaseType(user)
    if user.isStellarTerapagos?
      ret = :STELLAR
    else
      ret = :NORMAL
    end
