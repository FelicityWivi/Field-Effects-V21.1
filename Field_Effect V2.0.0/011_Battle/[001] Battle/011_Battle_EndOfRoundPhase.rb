class Battle
  ...
  def pbEOREndWeather(priority)
    ...
    @field.weatherDuration == 0 if %i[volcanic].any?{|f| is_field?(f)} && @field.weather == :Hail
  ...
  def pbEORWeatherDamage(battler)
    ...
    when :Sun, :HarshSun
      return if !%i[desert].any?{|f| is_field?(f)}
      return if !battler.pbHasType?(:GRASS) && !battler.pbHasType(:WATER)
      return if [:SOLARPOWER, :CHLOROPHYLL].include?(battler.ability_id)
      pbDisplay(_INTL("{1} was hurt by the sunlight!", battler.pbThis))
      amt = battler.totalhp / 8
    when :Sandstorm
      return if !battler.takesSandstormDamage?
      pbDisplay(_INTL("{1} is buffeted by the sandstorm!", battler.pbThis))
      if %i[desert].any?{|f| is_field?(f)}
        amt = battler.totalhp / 8
      else
    ...
    when :Hail
      ...
      if %i[frozendimensional].any?{|f| is_field?(f)}
        amt = battler.totalhp / 8
      else
        amt = battler.totalhp / 16
      end
...
#=============================================================================
  ...
  def pbEORTrappingDamage(battler)
    ...
    if battler.effects[PBEffects::Trapping] == 0
      ...
      battler.effects[PBEffects::SnapTrap] = false if battler.effects[PBEffects::SnapTrap]
      battler.effects[PBEffects::ThunderCage] = false if battler.effects[PBEffects::ThunderCage]
      battler.effects[PBEffects::Whirlpool] = false if battler.effects[PBEffects::Whirlpool]
      battler.effects[PBEffects::MagmaStorm] = false if battler.effects[PBEffects::MagmaStorm]
      battler.effects[PBEffects::FireSpin] = false if battler.effects[PBEffects::FireSpin]
      battler.effects[PBEffects::Infestation] = false if battler.effects[PBEffects::Infestation]
      battler.effects[PBEffects::SandTomb] = false if battler.effects[PBEffects::SandTomb]
      return
    end
    ...
    if battler.effects[PBEffects::SnapTrap] || battler.effects[PBEffects::ThunderCage] ||
       battler.effects[PBEffects::Whirlpool] || battler.effects[PBEffects::MagmaStorm] ||
       battler.effects[PBEffects::FireSpin] || battler.effects[PBEffects::Infestation] ||
       battler.effects[PBEffects::SandTomb]
    ...
    battler.pbConfuse if battler.pbCanConfuse?(battler, false, self) && %i[underwater].any?{|f| is_field?(f)}
    end
  end
  #=============================================================================
  ...
  def pbEndOfRoundPhase
    ...
    stallers=["none"]  
        for i in @battlers  
            stallers.push(i.name) if i.hasWorkingAbility(:STALL)  
        end  
        for count in 0...stallers.length    
            pbDisplay(_INTL("{1} stalls for time.",stallers[count])) if count>0  
    PBDebug.log("")
