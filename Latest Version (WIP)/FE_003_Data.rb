#===============================================================================
# Field Effect System – Field Data
#
# This is the user-editable data file. Every field is defined as an entry in
# the FIELDEFFECTS hash. The system compiles this hash into FE_DATA objects
# at the start of each battle (see 002_FieldEffect_Core.rb).
#
# HOW TO ADD A NEW FIELD
# ──────────────────────
# 1. Add a new key to FIELDEFFECTS below, following the same structure as an
#    existing field.
# 2. The key is a Ruby Symbol (e.g. :MYFIELD).
# 3. Add a graphic named "MyField" to Graphics/Battlebacks/ so the scene can
#    display it, and list it under :graphic.
# 4. Call battle.setField(:MYFIELD) from a move effect or script event.
#
# KEY REFERENCE
# ─────────────
# :name          → Display name string.
# :fieldMessage  → Array of intro messages (first is used by default).
# :graphic       → Array of battleback graphic filename strings.
# :secretPower   → Move ID string for Secret Power's effect on this field.
# :naturePower   → Move symbol used by Nature Power on this field.
# :mimicry       → Type symbol applied by Mimicry on this field.
#
# :damageMods    → { multiplier => [move_syms] }  0 = move fails.
# :accuracyMods  → { accuracy_pct => [move_syms] }  0 = always hits.
# :moveMessages  → { "Message" => [move_syms] }
# :moveEffects   → { "ruby_code" => [move_syms] }  (eval'd with 'battle' in scope)
# :typeMods      → { :TYPE => [move_syms] }  adds secondary type to those moves.
# :typeBoosts    → { multiplier => [:TYPE_SYMs] }
# :typeMessages  → { "Message" => [:TYPE_SYMs] }
# :typeCondition → { :TYPE => "condition_string" }  (eval'd in Move context; 'attacker'/'opponent' available)
# :typeEffects   → { "ruby_code" => [:TYPE_SYMs] }  (eval'd with 'battle' in scope)
# :changeCondition → { :NEW_FIELD_SYM => "condition_string" }
# :fieldChange   → { :NEW_FIELD_SYM => [move_syms] }
# :dontChangeBackup => [move_syms]  (these moves don't replace the backup layer)
# :changeMessage → { "Message" => [move_syms] }
# :changeEffects → { "ruby_code" => [move_syms] }  (eval'd with 'battle' in scope)
# :statusMods    → [move_syms]  (highlighted as field-boosted in UI)
# :seed          → { :seedtype, :effect, :duration, :message, :animation, :stats }
# :overlay       → Same sub-hash but applies when this field is a terrain overlay.
#
# Eval strings in :moveEffects / :typeEffects / :changeEffects run inside the
# Battle class with a local variable 'battle' pointing to the battle object.
# Use: battle.fe.counter, battle.FE, battle.setField(:X), etc.
#
# Eval strings in :typeCondition / :changeCondition run inside Battle::Move.
# Use: attacker, opponent, self (the move), @battle (the battle).
#===============================================================================
...
:INDOOR => {
	...
	:overlay => {		# effects of this field as an overlay instead of a full field 
		:damageMods => { #damage modifiers for specific moves, written as multipliers (e.g. 1.5 => [:TACKLE])
		},				# a damage mod of 0 denotes the move failing on this field
		:typeMods => {	# secondary types applied to moves (written as "type" => [move1,move2,....])
		},
...
:GRASSY => {
	...
	:typeAddOns => {},
	:moveEffects => {
		"battle.fe.counter += 1" => [:SURF],
		"battle.fe.counter += 2" => [:MUDDYWATER],
	},
	:typeBoosts => {
		1.5 => [:GRASS, :FIRE],
	},
...
:MISTY => {
	...
	:typeAddOns => {},
	:moveEffects => {
		"battle.fe.counter += 1" => [:CLEARSMOG, :SMOG, :POISONGAS],
		"battle.fe.counter = 2" => [:ACIDDOWNPOUR],
	},
	:typeBoosts => {
		1.5 => [:FAIRY],
		0.5 => [:DRAGON],
	...
	:typeEffects => {},
	:changeCondition => {
		:CORROSIVEMIST => "battle.fe.counter > 1",
	},
	:fieldChange => {
...
:ICY => {
	...
	:moveEffects => {
		...
		"battle.fe.counter += 1" => [:SCALD],
		"battle.fe.counter = 2" => [:STEAMERUPTION],
	},
	:typeBoosts => {
		1.5 => [:ICE],
		0.5 => [:FIRE],
	...
	:typeEffects => {},
	:changeCondition => {
		:INDOOR => "[:WATERSURFACE,:MURKWATERSURFACE].include?(battle.fe.backup) && (self.move!=:DIVE || battle.fe.counter == 3)",
		:WATERSURFACE => "battle.fe.counter > 1",
	},
	:fieldChange => {
...
:WATERSURFACE => {
	...
	:typeAddOns => {},
	:moveEffects => {
		"battle.fe.counter += 1" => [:SLUDGEWAVE],
		"battle.fe.counter = 2" => [:ACIDDOWNPOUR],
	},
	:typeBoosts => {
		1.5 => [:WATER, :ELECTRIC],
	...
	:typeEffects => {},
	:changeCondition => {
		:MURKWATERSURFACE => "battle.fe.counter > 1",
	},
	:fieldChange => {
...
:CAVE => {
	...
	:moveEffects => {
		...
		"battle.fe.counter2 += 1" => [:DRAGONPULSE],
		"battle.fe.counter2 = 2" => [:DRACOMETEOR, :DEVASTATINGDRAKE],
		"battle.fe.counter3 += 1" => [:FEVERPITCH, :MAGMADRIFT, :ERUPTION, :LAVAPLUME, :HEATWAVE, :OVERHEAT, :FUSIONFLARE],
		"battle.fe.counter4 += 1" => [:GRAVITY],
	},
	:typeBoosts => {
		1.5 => [:ROCK],
		0.5 => [:FLYING],
	...
	:typeEffects => {},
	:changeCondition => {
		:DRAGONSDEN => "battle.fe.counter2 > 1",
		:VOLCANIC => "battle.fe.counter3 > 1",
		:DEEPEARTH => "battle.fe.counter4 > 1",
	},
	:fieldChange => {
...
:CRYSTALCAVERN => {
	...
	:typeAddOns => {},
	:moveEffects => {
		"battle.fe.counter += 1" => [:EARTHQUAKE, :BULLDOZE, :MAGNITUDE, :FISSURE],
		"battle.fe.counter = 2" => [:TECTONICRAGE],
	},
	:typeBoosts => {
		# Standard field boosts. FIRE/WATER/GRASS/PSYCHIC added here so the
		# type-roll engine in 012_FieldEffect_RollMechanics.rb can change
		# eligible Rock moves to one of those types and the 1.5x applies automatically.
		1.5 => [:ROCK, :DRAGON, :FIRE, :WATER, :GRASS, :PSYCHIC],
	},
	:typeMessages => {
		"The crystals charged the attack!" => [:ROCK],
		"The crystal energy strengthened the attack!" => [:DRAGON],
		"The crystals crackled with fire!" => [:FIRE],
		"The crystals flowed with water!" => [:WATER],
		"The crystals flourished with life!" => [:GRASS],
		"The crystals pulsed with psychic energy!" => [:PSYCHIC],
	},
	:typeCondition => {},
	:typeEffects => {},
	:changeCondition => {
...
:DIMENSIONAL => {
	...
	:moveEffects => {
		"battle.fe.counter += 1" => [:BLIZZARD, :SHEERCOLD, :COLDTRUTH],
		"battle.fe.counter = 2" => [:ICEBURN, :FREEZESHOCK, :GLACIATE],
	},
	:typeBoosts => {
		1.5 => [:DARK, :SHADOW],
		1.2 => [:GHOST],
	...
	:typeEffects => {},
	:changeCondition => {
		:FROZENDIMENSION => "battle.fe.counter > 1",
	},
	:fieldChange => {
...
# ==============================================================================
# FIELDS PORTED FROM POKEMON REPUDIATION (not present in Rejuvenation fieldtxt)
# ==============================================================================

# ------------------------------------------------------------------------------
# Super-Heated Field [SUPERHEATED]
# Note: EOR passive damage (1/16 HP fire) is handled by 006_FieldEffect_EOR.rb.
# Steam mechanic (water moves → accuracy drop) fires via :typeEffects.
# Hail suppression is handled by noWeather in 003_FieldEffect_Battle.rb.
# Thrash/Outrage/Petal Dance 1-turn fatigue requires a 010-style override.
# ------------------------------------------------------------------------------
:SUPERHEATED => {
	...
	:typeMods => {
		:FIRE => [:ROCKSLIDE, :STONEEDGE, :ROCKBLAST, :ROCKTHROW, :HEADSMASH,
		          :ROCKWRECKER, :POWERGEM, :ANCIENTPOWER, :METEORBEAM, :STEELROLLER,
		          :SMACKDOWN, :ACCELEROCK, :ROLLOUT, :ROCKPOLISH, :WIDEGUARD],
	},
	:typeAddOns => {},
	:moveEffects => {},
	:typeBoosts => {
	...
	:typeMessages => {
		"The superheated field amplified the flames!" => [:FIRE],
		"The water was weakened by the heat..." => [:WATER],
		"The ice melted in the heat!" => [:ICE],
	},
	:typeCondition => {
		# Water nerf excludes Scald and Steam Eruption (the steam itself doesn't weaken)
		:WATER => "![:SCALD, :STEAMERUPTION].include?(@id)",
	},
	# Water moves trigger the steam accuracy drop after execution
	:typeEffects => {
		"battle.fieldAccuracyDrop" => [:WATER],
	},
	:changeCondition => {},
	:fieldChange => {},
	:dontChangeBackup => [],
...
# Blade/slash moves get ×1.5 + "The Knight is Justified!" message.
# Beam moves get ×1.4. Dark pulse group gets ×1.2.
# Accuracy of powder/spore moves is raised to 85%.
# ------------------------------------------------------------------------------
:ENCHANTEDFOREST => {
	...
	:mimicry => :POISON,
	:damageMods => {
		1.5 => [:HEX, :MYSTICALFIRE, :SPIRITBREAK, :MAGICALTORQUE, :FLEURCANNON,
		        :RELICSONG,
		        # blade/slash moves
		        :SLASH, :NIGHTSLASH, :PSYCHOCUT, :AIRSLASH, :FURYCUTTER, :XSCISSOR,
		        :SACREDSWORD, :LEAFBLADE, :RAZORSHELL, :CROSSPOISON, :AERIALACE,
		        :AIRCUTTER, :RAZORLEAF, :SOLARBLADE, :BEHEMOTHBLADE, :CEASELESSEDGE,
		        :STONEAXE, :AQUACUTTER, :FALSESWIPE, :FALSESURRENDER],
		1.4 => [:ICEBEAM, :HYPERBEAM, :AURORABEAM, :BUBBLEBEAM, :SIGNALBEAM,
		        :CHARGEBEAM, :SOLARBEAM, :PSYBEAM, :ORIGINPULSE, :MOONGEISTBEAM,
		        :FLEURCANNON, :ELECTROBEAM, :FOCUSBLAST, :FLASHCANNON],
	...
	:moveMessages => {
		"Magic aura amplified the attack!" => [:HEX, :MYSTICALFIRE, :SPIRITBREAK,
		                                        :MAGICALTORQUE, :FLEURCANNON, :RELICSONG,
		                                        :ICEBEAM, :HYPERBEAM, :AURORABEAM, :BUBBLEBEAM,
		                                        :SIGNALBEAM, :CHARGEBEAM, :SOLARBEAM, :PSYBEAM,
		                                        :ORIGINPULSE, :MOONGEISTBEAM, :ELECTROBEAM,
		                                        :FOCUSBLAST, :FLASHCANNON],
		"The Knight is Justified!" => [:SLASH, :NIGHTSLASH, :PSYCHOCUT, :AIRSLASH,
		                               :FURYCUTTER, :XSCISSOR, :SACREDSWORD, :LEAFBLADE,
		                               :RAZORSHELL, :CROSSPOISON, :AERIALACE, :AIRCUTTER,
		                               :RAZORLEAF, :SOLARBLADE, :BEHEMOTHBLADE, :CEASELESSEDGE,
		                               :STONEAXE, :AQUACUTTER, :FALSESURRENDER],
	...
	:typeMessages => {
		"The enchanted forest strengthened the attack!" => [:FAIRY, :GRASS, :POISON],
		"Darkness seeps through the ancient trees..." => [:DARK],
	},
	:typeCondition => {},
	:typeEffects => {},
	:changeCondition => {},
	...
	:seed => {
		...
		:effect => nil,
		:duration => nil,
		:message => "{1} was blessed by the enchanted forest!",
		:animation => :MOONBLAST,
		:stats => {
			:SPECIAL_DEFENSE => 1,
		},
...
# ------------------------------------------------------------------------------
# Sahara [SAHARA]
# Ice-type moves gain a Water secondary type.
# Water moves are weakened ×0.8.
# ------------------------------------------------------------------------------
:SAHARA => {
	:name => "Sahara",
	:fieldMessage => [
	...
	:mimicry => :GROUND,
	:damageMods => {
		1.5 => [:NEEDLEARM, :OVERHEAT, :PINMISSILE, :ROCKWRECKER, :SANDTOMB,
		        :SCORCHINGSANDS, :ATTACKORDER, :BUGBUZZ],
		0.8 => [:SURF, :WATERFALL, :HYDROPUMP, :AQUAJET, :SCALD, :MUDDYWATER,
		        :WATERPULSE, :BRINE, :CRABHAMMER, :LIQUIDATION, :WAVECRASH,
		        :WATERSPOUT, :ORIGINPULSE, :SPARKLINGARIA, :SOAK, :WHIRLPOOL,
		        :DIVE, :WATERGUN, :AQUATAIL],
	},
	:accuracyMods => {},
	:moveMessages => {
		"The arid terrain weakened the attack..." => [:SURF, :WATERFALL, :HYDROPUMP,
		                                              :AQUAJET, :SCALD, :MUDDYWATER,
		                                              :WATERPULSE, :BRINE, :CRABHAMMER,
		                                              :LIQUIDATION, :WAVECRASH, :WATERSPOUT,
		                                              :ORIGINPULSE, :SPARKLINGARIA, :SOAK],
	},
	# Ice-type moves gain a Water secondary type
	:typeMods => {
		:WATER => [:ICEBEAM, :BLIZZARD, :ICICLECRASH, :ICEPUNCH, :ICYSHARD,
		           :AURORABEAM, :FREEZEDRY, :GLACIALLANCE, :ICESPINNER,
		           :ICICLESPEAR, :POWDERSNOW, :FROSTBREATH, :TRIPLEAXEL],
	},
	:typeAddOns => {},
	:moveEffects => {},
	:typeBoosts => {
	...
	:typeMessages => {
		"The Sahara strengthened the attack!" => [:BUG, :FIRE, :GROUND, :ROCK],
	},
	:typeCondition => {},
	:typeEffects => {},
	:changeCondition => {},
	...
	:seed => {
		...
		:effect => nil,
		:duration => nil,
		:message => "{1} endured the scorching sands!",
...
# ------------------------------------------------------------------------------
# Poison Library [POISONLIBRARY]
# Grass-type moves gain a Poison secondary. Psychic-type moves gain a Fairy secondary.
# Poison boost only applies to grounded attackers.
# ------------------------------------------------------------------------------
:POISONLIBRARY => {
	:name => "Poison Library",
	:fieldMessage => [
	...
	:typeMods => {},
	:typeAddOns => {
		# All Grass-type moves gain a Poison secondary type
		:POISON => [:GRASS],
		# All Psychic-type moves gain a Fairy secondary type
		:FAIRY  => [:PSYCHIC],
	},
	:moveEffects => {},
	:typeBoosts => {
	...
	:typeMessages => {
		"The toxic knowledge empowered the attack!" => [:POISON],
		"The library's energy strengthened the attack!" => [:GRASS, :FIRE, :FAIRY],
	},
	:typeCondition => {
	...
	:seed => {
		...
		:effect => nil,
		:duration => nil,
		:message => "{1} absorbed knowledge from the library!",
		:animation => :ACID,
		:stats => {
			:SPECIAL_ATTACK => 1,
		},
...
# ------------------------------------------------------------------------------
# Canyon Field [CANYON]
# Rock secondary: Rock Climb, Earthquake, Magnitude, Strength, Bulldoze, Accelerock
# Grass secondary: Cut, Slash, Air Slash, Fury Cutter, Air Cutter, Psycho Cut, Breaking Swipe
# Active ability boosts (Rock Head, Sturdy, Steadfast, etc.) require 010-style overrides.
# ------------------------------------------------------------------------------
:CANYON => {
	:name => "Canyon Field",
	:fieldMessage => [
	...
	:damageMods => {
		2.0 => [:ROCKSMASH],
		1.5 => [:ROCKCLIMB, :STRENGTH, :MAGNITUDE, :EARTHQUAKE, :BULLDOZE,
		        :ACCELEROCK, :CUT, :SLASH, :AIRSLASH, :FURYCUTTER, :AIRCUTTER,
		        :PSYCHOCUT, :BREAKINGSWIPE, :ATTACKORDER],
	},
	:accuracyMods => {},
	:moveMessages => {
		"The canyon walls amplified the force!" => [:ROCKSMASH, :ROCKCLIMB, :STRENGTH,
		                                            :MAGNITUDE, :EARTHQUAKE, :BULLDOZE,
		                                            :ACCELEROCK],
		"The tangled vines guided the strike!" => [:CUT, :SLASH, :AIRSLASH, :FURYCUTTER,
		                                           :AIRCUTTER, :PSYCHOCUT, :BREAKINGSWIPE],
	},
	:typeMods => {
	...
	:typeMessages => {
		"The rocky canyon empowered the attack!" => [:ROCK, :BUG],
		"Vines and roots strengthened the attack!" => [:GRASS],
		"Hard canyon rock reinforced the strike!" => [:STEEL],
	},
	:typeCondition => {},
	:typeEffects => {},
	:changeCondition => {},
	...
	:dontChangeBackup => [],
	:changeMessage => {},
	:statusMods => [:ROCKPOLISH, :SANDSTORM, :STEALTHROCK, :STICKYWEB,
	                :DEFENDORDER, :GROWTH, :STRENGTHSAP, :HEALORDER, :FORESTSCURSE],
	:changeEffects => {},
	:seed => {
		:seedtype => :TELLURICSEED,
		:effect => :Endure,
		:duration => 1,
		:message => "{1} braced itself with canyon stone!",
		:animation => :ROCKPOLISH,
		:stats => {
			:DEFENSE => 1,
			:SPECIAL_DEFENSE   => 1,
...
# ------------------------------------------------------------------------------
# Magic Field [MAGIC]
# The Persona Roll (random multiplier by type) is implemented in
# 010_FieldEffect_MagicPersonaRoll.rb. This hash holds only the static data:
# fixed ×1.5 "magical" moves, accuracy mods, status mods, and seed.
# :typeBoosts is intentionally empty — Persona types are handled at runtime.
# The :typeEffects strings here are display-only markers used by the indicator;
# actual damage computation happens in the 010 override.
# ------------------------------------------------------------------------------
:MAGIC => {
	:name => "Magic Field",
	:fieldMessage => [
	...
	:graphic => ["Magic"],
	:secretPower => "AIRSLASH",    # Secret Power confuses (override in 010)
	:naturePower => :PSYCHIC,
	:mimicry => :PSYCHIC,
	:damageMods => {
		# Fixed ×1.5 magical-themed moves of non-Persona types
		1.5 => [:ANCIENTPOWER, :MAGICALLEAF, :FLOWERTRICK, :POWERGEM, :AURASPHERE,
		        :FOCUSBLAST, :HEX, :SECRETPOWER, :HIDDENPOWER,
		        :MOONBLAST,   # Moonblast rolls as Fairy in 010 — listed here as fallback
		        :MINDBLOWN],  # Mind Blown rolls as Fire in 010 — listed here as fallback
	},
	:accuracyMods => {
		90 => [:HYPNOSIS],
	},
	:moveMessages => {
		"The magical energy boosted the attack!" => [:ANCIENTPOWER, :MAGICALLEAF,
		                                             :FLOWERTRICK, :POWERGEM,
		                                             :AURASPHERE, :FOCUSBLAST, :HEX,
		                                             :SECRETPOWER, :HIDDENPOWER],
	},
	:typeMods => {},
	:typeAddOns => {},
	:moveEffects => {},
	# :typeBoosts intentionally empty — Persona Roll handled in 010
	:typeBoosts => {},
	:typeMessages => {},
	:typeCondition => {},
	:typeEffects => {},
	...
	:dontChangeBackup => [],
	:changeMessage => {},
	:statusMods => [:CALMMIND, :COSMICPOWER, :ESPERWING, :KINESIS, :MEDITATE,
	                :MINDREADER, :MIRACLEEYE, :PSYCHUP, :MYSTICALPOWER,
	                :PSYSHIELDBASH, :TELEKINESIS, :MAGICPOWDER, :HYPNOSIS,
	                :GRAVITY, :MAGICROOM, :TRICKROOM],
	:changeEffects => {},
	:seed => {
		:seedtype => :MAGICALSEED,
		:effect => :Confusion,
		:duration => 3,
		:message => "{1} was caught in a magical confusion!",
...
# PSYCHIC is an alias for PSYTERRAIN — added after the hash is fully defined.
FIELDEFFECTS[:PSYCHIC] = FIELDEFFECTS[:PSYTERRAIN]
